...
canvas.width = size;
canvas.height = size;
let c = new (0, _canvas.Canvas)(canvas);
let quadtree = new (0, _quadtree.QuadTree)(1, {
    x: 0,
    y: 0,
    height: size,
    width: size
...
for(let i = 0; i < 400; i++)quadtree.addPlayer({
    x: Math.floor(Math.random() * size),
    y: Math.floor(Math.random() * size),
    score: 0
}, (0, _uuid.v4)());
...
requestAnimationFrame(update); //c.drawRectangle({ height: , width: , x: , y:  }, "white")

},{"./src/canvas":"1Z0N5","./src/quadtree":"cO29y","uuid":"j4KJi"}],"1Z0N5":[function(require,module,exports) {
...
},{}],"cO29y":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class QuadTree {
    ...
    getPlayerByID(id) {
        if (!this.players[id]) {
            for (let node of this.nodes)return node.getPlayerByID(id);
        }
        return this.players[id];
    }
    addFruit(fruit, id) {
        if (!(0, _utils.contains)(this.bondary, fruit)) return false;
        ...
        return this.nodes.reduce((prev, curr)=>{
            return prev || curr.addFruit(fruit, id);
        }, false);
    }
    addPlayer(player, id) {
        if (!(0, _utils.contains)(this.bondary, player)) return false;
        ...
        if (this.nodes.length < 1) this.divide();
        return this.nodes.reduce((prev, curr)=>{
            return prev || curr.addPlayer(player, id);
        }, false);
    }
    getAllPlayers() {
        if (this.nodes.length < 1) return this.players;
        return this.nodes.reduce((prev, curr)=>{
            return {
                ...prev,
                ...curr.getAllPlayers()
    ...
    divide() {
        ...
        this.nodes[0] = new QuadTree(this.capacity, {
            x: this.bondary.x,
            y: this.bondary.y,
            width,
            height
        });
        this.nodes[1] = new QuadTree(this.capacity, {
            x: this.bondary.x + width,
            y: this.bondary.y,
            width,
            height
        });
        this.nodes[2] = new QuadTree(this.capacity, {
            x: this.bondary.x,
            y: this.bondary.y + height,
            width,
            height
        });
        this.nodes[3] = new QuadTree(this.capacity, {
            x: this.bondary.x + width,
            y: this.bondary.y + height,
            width,
            height
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["cWaoa","1jwFz"], "1jwFz", "parcelRequire2b51")
